...
var LOW_RISK_FILE_RE = (
  // Test files
  /_test\.go$|\.test\.[tj]sx?$|\.spec\.[tj]sx?$|test_.*\.py$|_test\.rs$|_bench\.rs$|_spec\.rs$|_spec\.rb$|(^|\/)(tests?|benches|__tests__|specs?)\//i
);
var LOW_RISK_FILE_EXTRA_RE = new RegExp(
  // Documentation
  "\\.md$|\\.ya?ml$|\\.json$|\\.toml$|\\.css$|yarn\\.lock$|package-lock\\.json$|Cargo\\.lock$|go\\.sum$|Gemfile\\.lock$|pnpm-lock\\.yaml$|composer\\.lock$|poetry\\.lock$|\\.svg$|\\.png$|\\.jpe?g$|\\.gif$|\\.ico$|\\.woff2?$|\\.eot$|\\.ttf$|(^|\\/)\\.gitignore$|(^|\\/)\\.gitattributes$|(^|\\/)\\.editorconfig$|(^|\\/)\\.prettierrc(\\.\\w+)?$|(^|\\/)\\.eslintignore$|(^|\\/)\\.dockerignore$|(^|\\/)LICENSE(\\.\\w+)?$|(^|\\/)\\.env\\.example$",
  "i"
);
