...
// src/signed-commit/signed-commit.ts
var CREATE_COMMIT_MUTATION = `
...
`;
async function createSignedCommit(octokit, options) {
  ...
  const hasDeletions = deletions !== void 0 && deletions.length > 0;
  ...
  if (baseRef) {
    ...
    if (force) {
      try {
        ...
        const status = err !== null && typeof err === "object" && "status" in err ? err.status : void 0;
        const errMessage = err !== null && typeof err === "object" && typeof err.message === "string" ? err.message : "";
        if (status === 404 || status === 422 && errMessage.includes("Reference does not exist")) {
          ...
          try {
            ...
            const deleteStatus = deleteErr !== null && typeof deleteErr === "object" && "status" in deleteErr ? deleteErr.status : void 0;
            if (deleteStatus !== 404) {
              const deleteMessage = deleteErr !== null && typeof deleteErr === "object" && typeof deleteErr.message === "string" ? deleteErr.message : String(deleteErr);
              throw new Error(
                `Failed to delete stale branch "${branch}" before recreating it (deleteRef status ${deleteStatus}: ${deleteMessage}). Original force-update error: ${errMessage}`,
                { cause: deleteErr }
              );
            }
          }
  ...
  const messageInput = { headline: message };
  if (body) {
  ...
  const response = await octokit.graphql(CREATE_COMMIT_MUTATION, { input });
...
function parseArgs(args) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === "--repo") {
      ...
      const val = args[++i];
      if (val === void 0) throw new Error("--add requires a file path argument");
      result.adds.push(val);
    } else if (arg === "--delete") {
      const val = args[++i];
      if (val === void 0) throw new Error("--delete requires a file path argument");
...
async function main() {
  ...
  function assertRelativePath(filePath, flag) {
    if (isAbsolute(filePath)) {
      throw new Error(
        `${flag} received an absolute path: "${filePath}". Git file paths must be relative to the repository root. Hint: use \`find dist/ -type f\` instead of \`find "$GITHUB_WORKSPACE/dist/" -type f\`.`
      );
    }
  }
