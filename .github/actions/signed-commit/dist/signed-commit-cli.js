...
function parseArgs(args) {
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        if (arg === '--repo') {
            ...
            const val = args[++i];
            if (val === undefined)
                throw new Error('--add requires a file path argument');
            result.adds.push(val);
        }
        else if (arg === '--delete') {
            const val = args[++i];
            if (val === undefined)
                throw new Error('--delete requires a file path argument');
...
async function main() {
    ...
    if (args.addFromStdin && args.deleteFromStdin) {
        throw new Error('--add-stdin and --delete-stdin cannot be used together (stdin can only be read once)');
    }
...
//# sourceMappingURL=signed-commit-cli.js.map
