...
│                                    #   Also exports DOCKER_AGENT_ACTION_ROOT (repo root of the downloaded action
│                                    #   copy; legacy alias CAGENT_ACTION_ROOT still exported) for subsequent
│                                    #   run: steps that need to invoke dist/ bundles.
│
├── .github/
│   ├── actions/
...
- **Bot reply detection** uses HTML markers: `<!-- docker-agent-review -->` on review comments, `<!-- docker-agent-review-reply -->` on agent replies (including mention-reply responses). The legacy `<!-- cagent-review -->` / `<!-- cagent-review-reply -->` markers are still recognized everywhere we detect (loop guards, stale-thread resolution, reply detection) during the repo-rename transition. **Don't change these strings** — workflows in consumer repos grep for them.
...
- Stale review threads on lines no longer in the diff are auto-resolved via GraphQL `resolveReviewThread`. Threads with no `<!-- docker-agent-review -->` marker (or the legacy `<!-- cagent-review -->` marker) are never touched.
...
- **Don't** rename markers (`<!-- docker-agent-review -->`, `<!-- docker-agent-review-reply -->`) without a versioned migration plan that keeps recognizing the prior markers until they age out (as the current `cagent-*` → `docker-agent-*` transition does).
